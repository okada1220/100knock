...
for info in basicinfo_findall:
    ...
    # ファイル除去
    sub_info = re.sub('\[{2}(.*?)\:(.*?)\|(.*?)\|(.*?)\]{2}', r'\1,\2,\3,\4', sub_info)

    # カテゴリ除去
    sub_info = re.sub('\[{2}(.*?)\:(.*?)\|(.*?)\]{2}', r'\1,\2,\3', sub_info)

    # リダイレクト除去
    sub_info = re.sub('\#(.*?)\[{2}(.*?)\#(.*?)\]{2}', r'\1,\2,\3', sub_info)
    sub_info = re.sub('\#(.*?)\[{2}(.*?)\]{2}', r'\1,\2', sub_info)

    # 内部リンク除去
    sub_info = re.sub('\[{2}(.*?)\#(.*?)\|(.*?)\]{2}', r'\1,\2,\3', sub_info)
    sub_info = re.sub('\[{2}(.*?)\|(.*?)\]{2}', r'\1,\2', sub_info)
    sub_info = re.sub('\[{2}(.*?)\]{2}', r'\1', sub_info)

    # 外部リンク除去
    sub_info = re.sub('\[(.*?)\]', r'\1', sub_info)
